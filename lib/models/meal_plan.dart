...
class MealPlan {
  ...
  factory MealPlan.fromMap(Map<String, dynamic> map) {
    final nutritionMap = map['nutrition_overview'] ??
        map['nutrition'] ??
        map['total_nutrition'] ??
        map['nutrition_breakdown'] ??
        map['total_macros'] ??
        map['daily_totals'] ??
        map['macros'];

    final parsedMeals = _parseMealsList(map['meals']);

    MealPlanNutritionBreakdown nutrition = MealPlanNutritionBreakdown.fromMap(
      nutritionMap is Map<String, dynamic> ? nutritionMap : <String, dynamic>{},
    );

    // Fallback: If total calories is 0 or missing in overview, compute sums from parsed meals
    if (nutrition.totalCalories <= 0 && parsedMeals.isNotEmpty) {
      double sumCal = 0;
      double sumProtein = 0;
      double sumCarbs = 0;
      double sumFat = 0;

      for (final m in parsedMeals) {
        sumCal += m.calories;
      ...
      final targetCal = _parseDouble(
        map['target_calories'] ?? map['targetCalories'],
      );

      nutrition = MealPlanNutritionBreakdown(
        totalCalories: sumCal > 0 ? sumCal : (targetCal > 0 ? targetCal : 0),
        protein: sumProtein,
        carbs: sumCarbs,
        fat: sumFat,
