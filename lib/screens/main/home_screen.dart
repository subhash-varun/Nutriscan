...
import 'package:nutriscan/screens/scanner/barcode_scanner_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
                ...
                children: [
                  ...
                  Expanded(
                    child: _buildImageSourceButton(
                      ...
                      label: 'Barcode',
                      subtitle: 'Package scan',
                      onTap: () async {
                        Navigator.pop(context);
                        await _scanBarcodeProduct();
                      },
                    ),
                  ),
                  const SizedBox(width: 8),
  ...
  Future<void> _scanBarcodeProduct() async {
    try {
      final scannedFood = await Navigator.push<Food>(
        context,
      ...
      if (scannedFood != null && mounted) {
        final foodProvider = context.read<FoodProvider>();
        await foodProvider.addScannedFood(scannedFood);

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Logged ${scannedFood.name} (${scannedFood.calories.round()} kcal)'),
