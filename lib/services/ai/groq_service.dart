...
class GroqService {
  ...
  static String _extractContentFromResponse(dynamic data) {
    ...
    } else if (rawContent is List && rawContent.isNotEmpty) {
      final textPart = rawContent.firstWhere(
        (p) => p is Map && p['type'] == 'text',
        orElse: () => rawContent[0],
      );
      if (textPart is Map && textPart['text'] is String) {
        content = (textPart['text'] as String).trim();
      } else {
        content = rawContent.join('\n').toString().trim();
      }
    }
    // Strip <think>...</think> tags output by reasoning models (even if unclosed)
    return content
...
- Respond with ONLY one valid JSON object. No markdown.''';
  ...
  Future<MealPlan> generateMealPlan({
    ...
    try {
      ...
      debugPrint('Groq API DioException: ${e.response?.statusCode} ${e.response?.data}');
      final msg = e.response?.data is Map && e.response?.data['error'] != null
          ? e.response?.data['error']['message']?.toString()
          : e.message;
      throw Exception(msg ?? getLocalizedErrorMessage('network_error_generic', language));
    } catch (e) {
      rethrow;
    }
  }
  ...
  String _getMealPlanPrompt({
    ...
    final langName = _languageNameForModel(language);
    final restrStr = restrictions.isNotEmpty ? "Dietary restrictions: ${restrictions.join(', ')}." : "";
    final contextStr = userNutritionContext != null && userNutritionContext.isNotEmpty
        ? "User profile: $userNutritionContext."
        : "";

    return '''Generate a complete $mealsPerDay-meal daily meal plan targeting $targetCalories kcal ($dietStyle diet style) in $langName. $restrStr $contextStr

Return ONLY a valid raw JSON object matching EXACTLY this JSON structure:
{
  "plan_title": "Plan title in $langName",
  "goal_summary": "Summary of macros and strategy in $langName",
  ...
  "meals": [
    {
      "type": "breakfast",
      "title": "Meal title in $langName",
      "description": "Short meal description in $langName",
      "calories": 500,
      "protein": 35,
      "carbs": 60,
      "fat": 12,
  ...
  "hydration_reminders": ["Hydration tip 1", "Hydration tip 2"],
  "lifestyle_tips": ["Lifestyle tip 1", "Lifestyle tip 2"],
  ...
  Future<Map<String, dynamic>> analyzeIngredientLabel(File imageFile, {String language = 'en'}) async {
    try {
      final base64Image = base64Encode(await imageFile.readAsBytes());
      final prompt = '''Analyze the ingredients label in this photo.
Identify harmful additives, preservatives, E-numbers, or trans fats.
Return ONLY a raw JSON object matching this structure:
{
  "safety_score": 85,
  "hazard_rating": "Clean",
  "additives_found": ["E250 (Sodium Nitrite)", "Artificial Red 40"],
  "summary": "Short 2 sentence safety summary in ${_languageNameForModel(language)}"
}''';
      ...
      return {
        "safety_score": 75,
        "hazard_rating": "Moderate Risk",
        "additives_found": ["Common food processing agents"],
        "summary": "Standard processed package product."
      };
    } catch (e) {
      return {
        "safety_score": 70,
        "hazard_rating": "Moderate Risk",
        "additives_found": [],
        "summary": "Unable to scan ingredient label clearly."
      };
    }
  }

  Future<List<Map<String, dynamic>>> getHealthySwaps(String foodName, {String language = 'en'}) async {
    try {
      final prompt = '''For the food item "$foodName", suggest 3 healthier alternative meals or recipe swaps that are higher in protein and lower in calories.
Return ONLY a raw JSON array of 3 objects matching this schema:
[
  {
    "swap_name": "Cauliflower Crust Pepperoni Pizza",
    "calorie_savings": "250 kcal lower",
    "protein_boost": "+14g protein",
    "description": "Short 1-sentence description in ${_languageNameForModel(language)}"
  }
]''';
      ...
      return [
        {
          "swap_name": "Grilled Chicken Salad",
          "calorie_savings": "200 kcal lower",
          "protein_boost": "+15g protein",
          "description": "Replace fried protein with seasoned grilled breast."
        }
      ];
    } catch (e) {
      return [];
  ...
  Future<bool> _checkNetworkConnectivity() async {
    try {
      ...
      // Return true to let Dio attempt the request directly if DNS lookup checks fail on emulator
      return true;
    }
  }
